...
public class Deductor {
   ...
   public static List<SquareState> fitCluesToGapsOnce(List<SquareState> existing, List<Integer> numbers) {
      List<Clue> clues = ChainedDeductor.createClues(numbers, existing.size());

      int gapStart = 0;

      for (int clueNr = 0; clueNr < clues.size(); clueNr++) {
         int firstFit = ChainedDeductor.findFirstFit(clues.get(clueNr).value, existing, gapStart);
         int offset = firstFit - gapStart;

         ChainedDeductor.offsetCluesToRight(clues, clueNr, offset);

         if (clues.get(clueNr).earliestEnd < existing.size() && existing.get(clues.get(clueNr).earliestEnd).isFilled()) {
            offset++;
            ChainedDeductor.offsetCluesToRight(clues, clueNr, 1);
         }
